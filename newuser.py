...
WHERE gu.USER_ID IS NULL AND up.USER_ID IS NULL
ORDER BY au.NAME;
"""
...
SELECT
    ...
    g.DESCRIPTION,
    g.SITE_ID,
    (SELECT COUNT(*) FROM GROUP_USER gu WHERE gu.GROUP_ID = g.ID) AS MemberCount,
...
def api_users():
    ...
    if detailed:
        ...
        return jsonify(rows)
    rows = run_query("SELECT NAME FROM APPLICATION_USER ORDER BY NAME;")
